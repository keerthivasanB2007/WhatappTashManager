...
export default function TopBar() {
  ...
  return <header className="global-topbar"><div className="crumb"><span>Workspace</span><em className="crumb-sep">/</em><strong>Task dashboard</strong></div><label className="global-search"><span>⌕</span><input ref={input} value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder="Search tasks, messages, or senders"/><kbd>Ctrl K</kbd></label><div className="top-actions"><button title="Toggle insights" className="top-action" onClick={() => setRailHidden(!railHidden)}>▤</button><div className="profile-menu" ref={profile}><button className="profile-trigger" onClick={() => setMenuOpen(open => !open)} aria-expanded={menuOpen}><span className="avatar">K</span><span className="profile-name">Keerthivasan</span><span className="profile-caret">⌄</span></button>{menuOpen && <div className="profile-popover"><div className="profile-popover-head"><span className="avatar">K</span><div><strong>Keerthivasan</strong><small>Task dashboard</small></div></div><button className="logout-option" onClick={logout}>Log out <span>→</span></button></div>}</div></div></header>;
}
